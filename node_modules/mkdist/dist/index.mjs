import { basename, resolve, extname, join, dirname } from 'pathe';
...
const vueLoader = async (input, context) => {
  ...
  for (const blockLoader of [sassLoader$1, scriptLoader]) {
...
const sassLoader$1 = vueBlockLoader({
  outputLang: "css",
  type: "style"
});
...
const defaultLoaders = ["js", "vue", "sass"];
...
function resolveLoaders(loaders2 = defaultLoaders) {
  return loaders2.map((loader) => resolveLoader(loader)).filter(Boolean);
}
